...
export default function Share() {
	...
	const handleFetch = () => {
		if (redditUrl) {
			const { subreddit, postId } = extractSubredditAndPostId(redditUrl);
			setRedditData({ subreddit: subreddit, postId: postId });
			console.log(redditData);
		}
	};

	const extractSubredditAndPostId = (url) => {
		...
		if (match) {
			...
			return null; // Invalid Reddit URL
		}
	};
	...
	const handleMint = async () => {
		...
		try {
			...
			console.log("listing price is " + listing);
			setListingPrice(listing);
		} catch (error) {
			alert("error in fetching the price ", error);
		}
		...
		console.log("ipfs url" + url, "price:" + Price, listingPrice.toString());
		...
		<div className="share">
			<div className="shareWrapper">
				<div className="shareTop">
					...
					/>
					<input
						placeholder="redditUrl"
						className="shareInput"
						value={redditUrl}
						onChange={handleRedditUrl}
					/>
